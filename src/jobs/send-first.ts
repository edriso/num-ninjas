...
import { getOrCreateTodaySession } from '../services/session.service.js';
...
export async function sendFirstQuestion(bot: Bot<BotContext>) {
  ...
  for (const account of accounts) {
    ...
    try {
      ...
      // Import the question service directly
      const { getNextQuestion } = await import('../services/question.service.js');
      ...
      const { markQuestionSent } = await import('../services/session.service.js');
