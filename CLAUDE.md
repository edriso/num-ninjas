...
- **Block tracking**: `accounts.blocked_at` mirrors whether the user has blocked the bot, so every outbound cron filters at the SQL level (`where: { blockedAt: null }`) instead of wasting Telegram API calls on guaranteed 403s. Set in real-time via the `my_chat_member` handler in `bot/index.ts`, with a fallback in `bot/helpers/send-errors.ts` that catches "bot was blocked" 403s and sets it (handles missed events while the bot was offline). Cleared when the user interacts (session middleware) or unblocks via `my_chat_member`. **Important**: `'my_chat_member'` is NOT in Telegram's default `allowed_updates` — `bot.start({ allowed_updates: [...] })` lists it explicitly, and any future update types must be added there too (Telegram replaces, not merges).
