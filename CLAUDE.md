...
- **Adapter pattern for DB**: MySQL via @prisma/adapter-mariadb, isolated in `packages/database/src/client.ts`. Pool configured with short idle timeout (60s) and `minimumIdle: 0` because Hostinger shared hosting kills idle connections. Connections are created on demand and validated before use.
...
- **Hostinger kills idle DB connections**: Shared hosting drops MySQL connections after a short idle period. The mariadb pool in `client.ts` uses `idleTimeout: 60` and `minimumIdle: 0` to handle this — connections are created on demand, not kept open.
